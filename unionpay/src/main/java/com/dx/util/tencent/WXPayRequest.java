...
import com.dx.util.tencent.BaseHttpSSLSocketFactory.TrustAnyHostnameVerifier;
...
public class WXPayRequest {
	 ...
	 */
	public WXPayRequest(String url, int connectionTimeout, int readTimeOut) {
		try {
	...
	public int send(Map<String, String> data, String encoding) throws Exception {
		try {
			...
			String sendData = WXPayUtil.mapToXml(data);
			WXPayUtil.getLogger().info("请求报文:[" + sendData + "]");
			this.requestServer(httpURLConnection, sendData,
					encoding);
			this.result = this.response(httpURLConnection, encoding);
			WXPayUtil.getLogger().info("Response message:[" + result + "]");
	...
	public int sendGet(String encoding) throws Exception {
		try {
			...
			this.result = this.response(httpURLConnection, encoding);
			WXPayUtil.getLogger().info("同步返回报文:[" + result + "]");
			return httpURLConnection.getResponseCode();
		} catch (Exception e) {
			throw e;
		}
	...
	private String response(final HttpURLConnection connection, String encoding)
		...
		try {
			...
			WXPayUtil.getLogger().info("HTTP Return Status-Code:["
	...
	private HttpURLConnection createConnection(String encoding) throws ProtocolException {
		...
		if ("https".equalsIgnoreCase(url.getProtocol())) {
			...
			if(!WXPayConstants.isIfValidateRemoteCert){
